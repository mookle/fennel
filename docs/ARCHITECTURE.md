...
- **`order`**: the Elixir service that owns obligation. It creates one Order per shop from a Purchase, and it runs the order state machine to acceptance. It never reads `product`.
...
- **OrderSku**: the immutable crystallised snapshot of an SKU at the moment of submission. Nobody can edit it, and it has no status (ADR-0004). `docs/services/order.md` lists its fields.

**The facts.**

- **Placed**: a buyer fact, the record that the buyer submitted. `order` sets it on each Order it creates from the shop split (ADR-0005).
...
Three services connect through two events and one synchronous read path. The services share nothing: no common database and no identity service. `shop_id` and `user_id` are opaque identifiers.

`cart` and `order` are **separate Mix projects**, not co-located under the same umbrella app. Each side duplicates the event envelope and the payload structs instead of sharing a library. Each side owns its own view of the wire format, and `contracts/events.md` is the shared truth (ADR-0014). Each service has its own database and will never query the database of another service directly.

**Cart to Order (intent to obligation).** Checkout ends at submission. It mints a customer-facing `purchase_id` and publishes one `purchase.submitted` fact to the broker. `order` consumes that fact, splits it into one Order per shop, and mints each `order_id`. From that point `order` owns everything durable. Neither service names the other's resources, and only `purchase_id` crosses (ADR-0013, ADR-0011).

**Cart reads Product.** `cart` reads from `product` synchronously over REST: SKU lookup, catalogue query, and the shipping quote. `cart` is the only consumer of `product`. Checkout crystallises everything it reads into an immutable snapshot, so a later product edit never changes a historical order (ADR-0004).
...
│   ├── cart/                   # Elixir project: memory-first cart, checkout, Purchase
│   └── order/                  # Elixir project: shop split, Order, state machine
...
These rules bind every schema and every payload in the build, and this section is the source of truth for them. `CLAUDE.md` repeats the four with the most expensive default, and it defers here for the rest and for the reasons. The set predates this build, and ADR-0017, ADR-0018, ADR-0019 and ADR-0024 have since decided the four that were load-bearing and unrecorded.

- Text is UTF-8. Prefer correct i18n and sorting over micro-optimisation.
- Money is `NUMERIC(15,4)`. JSON carries the amount as a **string** to keep the precision, padded to exactly four decimal places on output. An amount and its ISO-4217 `currency` (ADR-0030) are one value: on the wire it is an object of `amount` and `currency`, in memory it is one type, and the column holds the amount alone, with the read composing the pair from the product's currency (ADR-0032). A response carries both halves or neither. Never do arithmetic across two currencies. Nothing in this build rounds, because every operand is at scale 4 and every quantity is an integer. ADR-0017 holds the rule for the day that changes. The product holds the `currency` for every SKU beneath it, as a stand-in for the shop, which is not an entity in this build (ADR-0028, ADR-0029). Zero is an amount, not an absence. A value that can be absent is of type NullMoney (ADR-0031).
